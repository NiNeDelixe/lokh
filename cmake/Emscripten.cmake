...
set(lokh_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(lokh_COI_WORKER "${lokh_WEB_DIR}/coi-serviceworker.min.js")
set(lokh_SHELL_TEMPLATE "${lokh_WEB_DIR}/shell_template.html.in")
set(lokh_INDEX_TEMPLATE "${lokh_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(lokh_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(lokh_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(lokh_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(lokh_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY lokh_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY lokh_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY lokh_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY lokh_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE lokh_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${lokh_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${lokh_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${lokh_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${lokh_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${lokh_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${lokh_COI_WORKER}")
...
function(lokh_create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY lokh_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY lokh_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY lokh_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${lokh_INDEX_TEMPLATE}")
    configure_file("${lokh_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${lokh_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY lokh_WASM_TARGET_${target}_OUTPUT_NAME)
